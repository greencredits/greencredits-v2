...
// --------------------------------------------
// STRICT GEOFENCING SETTINGS
// --------------------------------------------
export const DEV_MODE = true; // ⚠️ SET TO FALSE FOR PRODUCTION
...
export function detectZoneFromCoordinates(lat, lng) {
  ...
  // 1. Check Strict Bounds (Ignore in Dev Mode)
  if (!DEV_MODE) {
    if (lat < GONDA_BOUNDS.minLat || lat > GONDA_BOUNDS.maxLat ||
      lng < GONDA_BOUNDS.minLng || lng > GONDA_BOUNDS.maxLng) {
      console.log(`📍 Rejected: ${lat}, ${lng} is outside Gonda City.`);
      return null; // OUTSIDE SERVICE AREA
    }
  } else {
    console.log('🚧 DEV_MODE: Ignoring GPS bounds check.');
  }

  // 2. Assign Internal Zones
  if (lat > 27.145) return 'Zone 1 - North Gonda';
  if (lat < 27.125) return 'Zone 2 - South Gonda';
  if (lng > 81.970) return 'Zone 3 - East Gonda';
  if (lng < 81.950) return 'Zone 4 - West Gonda';
